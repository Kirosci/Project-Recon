...
passiveEnumeration(){
    ...
    (
        if [ -f ".tmp/subdomains/passive/assetfinderSubdomains.txt" ]; then
            # echo -e $(printf "%s%s%s" "\t\t|---" "${GREEN}" "[Assetfinder results are already there: " "$(cat '.tmp/subdomains/passive/assetfinderSubdomains.txt' | wc -l)" "]" "${RESET}" "\t$time") 
            echo -ne "\t\t|---$GREEN[Assetfinder results are already there: $(cat '.tmp/subdomains/passive/assetfinderSubdomains.txt' | wc -l)]$RESET\t\t$time"
        else
    ...
    (
        if [ -f ".tmp/subdomains/passive/haktrailsSubdomains.txt" ]; then
            echo -ne "\t\t|---$GREEN[Haktrails results are already there: $(cat '.tmp/subdomains/passive/haktrailsSubdomains.txt' | wc -l)]$RESET\t\t$time"
        else
    ...
    (
        if [ -f ".tmp/subdomains/passive/subfinderSubdomains.txt" ]; then
            echo -ne "\t\t|---$GREEN[Subfinder results are already there: $(cat '.tmp/subdomains/passive/subfinderSubdomains.txt' | wc -l)]$RESET\t\t$time"
        else
    ...
    (
        if [ -f ".tmp/subdomains/passive/subdominatorSubdomains.txt" ]; then
            echo -ne "\t\t|---$GREEN[Subdominator results are already there: $(cat '.tmp/subdomains/passive/subdominatorSubdomains.txt' | wc -l)]$RESET\t\t$time"
        else
    ...
    (
        if [ -f ".tmp/subdomains/passive/amassSubdomains.txt" ]; then
            echo -ne "\t\t|---$GREEN[Amass results are already there: $(cat '.tmp/subdomains/passive/amassSubdomains.txt' | wc -l)]$RESET\t\t$time"
            # echo -e $(printf "%s%s%s" "\t\t|---" "${GREEN}" "[Assetfinder results are already there: " "$(cat '.tmp/subdomains/passive/amassSubdomains.txt' | wc -l)" "]" "${RESET}" "\t$time") 
            ...
            if [[ "$2" -eq 1 ]]; then   
                ...
                echo -e $(printf "%s%s%s" "\t\t|---" "${GREEN}" "[Amass: " "$(cat amassSubdomains.txt | wc -l)" "]" "${RESET}" "\t$time") 
                # echo -e "\t\t|---${GREEN}[Amass: $(cat amassSubdomains.txt | wc -l)]${RESET} \t$time" 
            elif [[ "$2" -eq 0 ]]; then 
                echo -e $(printf "%s%s%s" "\t\t|---" "${RED}" "[Skipping Amass]" "${RESET}" "\t$time") 
                # echo -e "\t\t|---${RED}[Skipping Amass]${RESET} \t$time"    
            else    
                amass enum -d "$domain" -timeout $timeout -o amassSubdomains.txt 2> /dev/null
                echo -e $(printf "%s%s%s" "\t\t|---" "${GREEN}" "[Amass: " "$(cat amassSubdomains.txt | wc -l)" "]" "[Timeout: " "$2" " minutes]" "${RESET}" "\t$time") 
